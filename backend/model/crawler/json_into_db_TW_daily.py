...
# terminal 執行: python -m backend.crawler.json_into_db_TW_daily

# 假設你從某處獲得這個 JSON 字串
filename = 'fetch_TW_books_daily_250528.json'
with open(filename, "r", encoding="utf-8") as f:
    book_list = json.load(f)

BOOKSTORE_NAME = 'books'

def get_or_create_author(name_tw, name_original):
    query = "SELECT id FROM authors WHERE name_tw = %s"
    result = db_pool.get_cursor(query, (name_tw,), fetch=True)
    
    if result:
        return result[0]['id']

    insert = """
        INSERT INTO authors (name_original, name_tw)
        VALUES (%s, %s)
    """
    db_pool.insert_cursor(insert, (name_original, name_tw))
...
def insert_book_if_not_exists(book_data, bookstore_id, author_id, published_date):
    ...
    insert_query = """
        ...
        image_url, book_url, isbn, publisher, published_date, price, description,
        language)
...
for book_data in book_list:  
    ...
    if not (2 <= len(book_data["categories"]) <= 4):
