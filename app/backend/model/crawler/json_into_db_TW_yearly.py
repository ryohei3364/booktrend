...
# terminal 執行: python -m backend.crawler.json_into_db_TW_yearly

# 假設你從某處獲得這個 JSON 字串
filename = 'fetch_TW_books_year-1.json'
with open(filename, "r", encoding="utf-8") as f:
    book_list = json.load(f)

BOOKSTORE_NAME = 'books'

def get_or_create_author(name, name_original):
    query = "SELECT id FROM authors WHERE name = %s"
    result = db_pool.get_cursor(query, (name,), fetch=True)
    
    if result:
        return result[0]['id']

    insert = """
        INSERT INTO authors (name_original, name, language)
        VALUES (%s, %s, %s)
    """
    db_pool.insert_cursor(insert, (name_original, name, 'zh-TW'))
